...
function Cria_Index(json) {
  for (let element in json["data"]) {
    const div = `
        <div class="champion" 
            ...
            data-tags="${json["data"][element]["tags"].reduce(
              (i, j) => i + "." + j
            )}"
            data-json="${json["data"][element]["id"]}.json">
            <div class="img" style='background-image:url("https://ddragon.leagueoflegends.com/cdn/img/champion/loading/${
              json["data"][element]["id"]
            }_0.jpg")'></div>
            <div class="all-text">
            <div class="Champion-Name"><h3>${
              json["data"][element]["name"]
            }</h3></div>
  ...
  AllChampions.map((i) =>
    i.addEventListener("click", function (event) {
      fetch(
        `https://ddragon.leagueoflegends.com/cdn/${version}/data/pt_BR/champion/${i.dataset.json}`
      )
        .then((res) => res.json())
...
function Champions_Res(json) {
  fetch(
    `https://ddragon.leagueoflegends.com/cdn/${json}/data/pt_BR/champion.json`
  )
    .then((r) => r.json())
...
function inputs_checked(checkeds, div) {
  if (checkeds.length == 2) {
    if (
      checkeds.includes(div.dataset.tags.split(".")[0]) &&
      checkeds.includes(div.dataset.tags.split(".")[1])
    ) {
      return true;
    } else {
      return false;
...
function Champion_Filter(letra) {
  AllChampions.forEach((i) => {
    if (
      Filtro_Letras(letra, i.dataset.name.toLowerCase()) &
      inputs_checked(
        inputs_tags
...
async function Cria_Champion(json, id) {
  window.scrollTo({
    behavior: "auto",
    top: 0,
    left: 0,
  });
  const Array_Habilities = json["data"][id]["spells"].map(
    (i) => i["image"]["full"]
  );
  main.setAttribute(
    "style",
  ...
  json["data"][id]["skins"].map((i) =>
    skins.insertAdjacentHTML(
      "beforeend",
      `
    <div class="Skin"><a href = "https://ddragon.leagueoflegends.com/cdn/img/champion/splash/${
      json["data"][id]["id"]
    }_${i["num"]}.jpg" data-lightbox="${json["data"][id]["id"]}" data-title="${
        i["name"] != "default" ? i["name"] : json["data"][id]["name"]
      }">
    <img src="https://ddragon.leagueoflegends.com/cdn/img/champion/splash/${
      json["data"][id]["id"]
    }_${i["num"]}.jpg" data-toggle="tooltip" title="${
        i["name"] != "default" ? i["name"] : json["data"][id]["name"]
      }">
    </a>
    </div>`
  ...
  // const res = await fetch(`https://ddragon.leagueoflegends.com/cdn/${version}/data/pt_BR/item.json`)
  // const json_items = await res.json()
  // InserItems(json_Items)
...
function InserItems(json_Items) {
  ...
  const Recomendado_Situacionais = document.querySelector(
    ".Items_Situacionais"
  );
  let items_add;
  items_add = items_SR.filter((i) => i["type"] == "starting")[0];
  if (items_add != null) {
    items_add["items"].forEach((i, j) =>
      (i["id"] == "3007") |
      (i["id"] == "3008") |
      (i["id"] == "3029") |
      (i["id"] == "3073")
        ? ""
        : Recomendado_Inicial.insertAdjacentHTML(
            "beforeend",
            `<img src="https://ddragon.leagueoflegends.com/cdn/${version}/img/item/${
              i["id"]
            }.png" data-toggle='tooltip' data-html='true' title="${
              json_Items["data"][i["id"]]["description"].length > 500
                ? json_Items["data"][i["id"]]["plaintext"]
                : json_Items["data"][i["id"]]["description"]
            }">`
          )
    );
  }
  ...
  if (items_add != null) {
    items_add["items"].forEach((i, j) =>
      ...
      (i["id"] == "3073")
        ? ""
        : Recomendado_Essencial.insertAdjacentHTML(
            "beforeend",
            `<img src="https://ddragon.leagueoflegends.com/cdn/${version}/img/item/${
              i["id"]
            }.png" data-toggle='tooltip' data-html='true' title="${
  ...
  items_add = items_SR.filter(
    (i) =>
      (i["type"] == "early") & (i["showIfSummonerSpell"] != "SummonerSmite")
  )[0];
  if (items_add != null) {
    items_add["items"].forEach((i, j) =>
      (i["id"] == "3007") |
      ...
      (i["id"] == "3073")
        ? ""
        : Recomendado_Inicio.insertAdjacentHTML(
            "beforeend",
            `<img src="https://ddragon.leagueoflegends.com/cdn/${version}/img/item/${
              i["id"]
            }.png" data-toggle='tooltip' data-html='true' title="${
  ...
  items_add = items_SR.filter(
    (i) =>
      (i["type"] == "offensive") & (i["hideIfSummonerSpell"] != "SummonerSmite")
  )[0];
  if (items_add != null) {
    items_add["items"].forEach((i, j) =>
      (i["id"] == "3007") |
      ...
      (i["id"] == "3073")
        ? ""
        : Recomendado_Situacionais.insertAdjacentHTML(
            "beforeend",
            `<img src="https://ddragon.leagueoflegends.com/cdn/${version}/img/item/${
              i["id"]
            }.png" data-toggle='tooltip' data-html='true' title="${
  ...
  items_add = items_SR.filter(
    (i) =>
      (i["type"] == "defensive") & (i["hideIfSummonerSpell"] != "SummonerSmite")
  )[0];
  if (items_add != null) {
    items_add["items"].forEach((i, j) =>
      (i["id"] == "3007") |
  ...
  document
    .querySelector(".Recommended_Builds")
    .setAttribute("class", "Recommended_Builds border-right_eu");
  ...
  if (items_add != null) {
    ...
    document
      .querySelector(".Recommended_Builds_JG")
      .setAttribute("class", "Recommended_Builds_JG border-left_eu");
    document.querySelector(".Recommended_Builds_JG").style.padding = "10px";
    const Recomendado_Inicial_JG = document.querySelector(
      ".Items_Recomended_JG"
    );
    const Recomendado_Inicio_JG = document.querySelector(".Items_Inicio_JG");
    const Recomendado_Essencial_JG = document.querySelector(
      ".Items_Essenciais_JG"
    );
    const Recomendado_Situacionais_JG = document.querySelector(
      ".Items_Situacionais_JG"
    );
    items_add["items"].forEach((i, j) =>
      (i["id"] == "3007") |
      (i["id"] == "3008") |
      ...
      (i["id"] == "3073")
        ? ""
        : Recomendado_Inicial_JG.insertAdjacentHTML(
            "beforeend",
            `<img src="https://ddragon.leagueoflegends.com/cdn/${version}/img/item/${
              i["id"]
            }.png" data-toggle='tooltip' data-html='true' title="${
    ...
    items_add = items_SR.filter(
      (i) =>
        (i["type"] == "essentialjungle") &
        (i["showIfSummonerSpell"] == "SummonerSmite")
    )[0];
    if (items_add != null) {
      items_add["items"].forEach((i, j) =>
        (i["id"] == "3007") |
        ...
        (i["id"] == "3073")
          ? ""
          : Recomendado_Essencial_JG.insertAdjacentHTML(
              "beforeend",
              `<img src="https://ddragon.leagueoflegends.com/cdn/${version}/img/item/${
                i["id"]
              }.png" data-toggle='tooltip' data-html='true' title="${
    ...
    if (items_add != null) {
      items_add["items"].forEach((i, j) =>
        ...
        (i["id"] == "3073")
          ? ""
          : Recomendado_Inicio_JG.insertAdjacentHTML(
              "beforeend",
              `<img src="https://ddragon.leagueoflegends.com/cdn/${version}/img/item/${
                i["id"]
              }.png" data-toggle='tooltip' data-html='true' title="${
    ...
    if (items_add != null) {
      items_add["items"].forEach((i, j) =>
        ...
        (i["id"] == "3073")
          ? ""
          : Recomendado_Situacionais_JG.insertAdjacentHTML(
              "beforeend",
              `<img src="https://ddragon.leagueoflegends.com/cdn/${version}/img/item/${
                i["id"]
              }.png" data-toggle='tooltip' data-html='true' title="${
    ...
    items_add = items_SR.filter(
      (i) =>
        (i["type"] == "aggressive") &
        ((i["showIfSummonerSpell"] == "SummonerSmite") |
          ((i["showIfSummonerSpell"] == "") & (i["hideIfSummonerSpell"] == "")))
    )[0];
    if (items_add != null) {
      items_add["items"].forEach((i, j) =>
        (i["id"] == "3007") |
